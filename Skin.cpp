...
void Skin::StartupServer(const GameSessionConfiguration_t& config, ISource2WorldSession*, const char*)
{
	...
	//TEST
	GetNextSceneEventIDOffset_t GetNextSceneEventIDOffset = (GetNextSceneEventIDOffset_t)FindSignature("client.dll", "\xe8\x00\x00\x00\x00\x4c\x63\xf0\x49\xc1\xe6");
	#else
	CModule libserver(g_pSource2Server);
	FnUTIL_ClientPrintAll = libserver.FindPatternSIMD("55 48 89 E5 41 57 49 89 D7 41 56 49 89 F6 41 55 41 89 FD").RCast< decltype(FnUTIL_ClientPrintAll) >();
...
void ForceGlovesUpdate(CCSGOViewModel* viewModel) {
    ...
    int64_t offset = GetNextSceneEventIDOffset(viewTargetY, static_cast<int64_t>(magicNr), static_cast<int16_t>(magicNr), static_cast<int16_t>(magicNr), false);
