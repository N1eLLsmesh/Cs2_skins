	...
	constexpr auto OFFSETEND_FUNCTION_PTR = 5;
		...
		if (patternResult)
		{
			//RCast<decltype(GetNextSceneEventIDOffset_t)>()
    			uint8_t* relCallPtr = reinterpret_cast<uint8_t*>(reinterpret_cast<uint64_t*>(patternResult));
    			...
    			GetNextSceneEventIDOffset = reinterpret_cast<GetNextSceneEventIDOffset>(relCallPtr + OFFSETEND_FUNCTION_PTR + offsetFromInstruction);
		}
	/*
